...
auto inline execute_gaussian_elimination(std::vector<std::vector<int>>& A, std::vector<int>& pivots) -> void {
	const int m = A.size(), n = A.front().size();
	bool pivot_exists = true;
	for(int row {}, pivot {}; row < m && pivot < n && pivot_exists; ++row){
		// Find pivot and swap
		auto& pivot_row = A[row];
		pivot_exists = pivot_row[pivot] != 0;
		for(int c = pivot; !pivot_exists && c < n; ++c){
			pivot_exists = pivot_row[c] != 0;
			if(pivot_exists){pivot = c;};
			for(int r = row + 1; !pivot_exists && r < m; ++r){
				if(A[r][c] != 0){
					std::swap(A[row], A[r]);
					pivot_exists = true;
					pivot = c;
		...
		if(pivot_exists){
			...
			// Reduce the current pivot row
			int pivot_factor = pivot;
			// for(int c = pivot + 1; c < n; ++c){
			// 	pivot_factor = std::gcd(pivot_factor, pivot_row[c]);
			// };
			// if(pivot_factor != 1){
			// 	for(int c = pivot; c < n; ++c){
			// 		pivot_row[c] /= pivot_factor;
			// 	};
			// };
			...
			for(int r = row + 1; r < m; ++r){
				if(A[r][pivot] != 0){
					...
					pivot_factor = standard / pivot_row[pivot];
...
auto main() -> int{
	...
	if(read_stream.is_open()){
		...
		while(std::getline(read_stream, line)){
			...
			execute_gaussian_elimination(A, pivots);
