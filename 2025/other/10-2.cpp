...
auto inline get_init_bounds(const int n, const std::vector<std::vector<int>>& A, const int pivots, const int x_var, const int max_power) -> std::pair<int,int> {
	...
	std::cout << "og bounds\t:\tx" << x_var << " -> [" << bounds.first << ", " << bounds.second << "]\n";
	for(int r = pivots - 1; r > -1; --r){
		if(A[r][x_var] != 0){
			bool is_unique = true;
	...
	std::cout << "updated bounds\t:\tx" << x_var << " -> [" << bounds.first << ", " << bounds.second << "]\n";
	return bounds;
};
...
auto inline multi_back_substitution(const int n, const std::vector<std::vector<int>>& A, const int pivots, const int max_power) -> int {
	...
	std::cout << "\n";
	std::cout << "Pivots:\t" << pivots << "\n";
	std::cout << "Free Variables:\t" << n - pivots - 1 << "\n";
...
auto main() -> int{
	...
	if(read_stream.is_open()){
		...
		while(std::getline(read_stream, line)){
			...
			// int val = pivots == n - 1 ? 0: multi_back_substitution(n, A, pivots,max_power);
			if(pivots == n - 1){std::cout << "UNIQUE SOLUTION DETECTED\n";}; 
